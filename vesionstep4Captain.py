...
def send_command(shipId, command):
    ...
    if command != "exit":
        ...
        if any(s["pos"] == new_pos for sid, s in ship_dict.items() if sid != shipId):    #si hay algún ship ya con la misma pos, colision
            print(f"Move {command} for ship {shipId} is not possible due to collision.", file=sys.stderr)
            sys.stderr.flush()
            return
        print(f"Ship {shipId} new position: {new_pos}", file=sys.stderr)
        sys.stderr.flush()
    try:
        os.write(shipId_dict["w_pipe"], f"{command}\n".encode())   #envía el command (up, down, left, right) desde w_pipe. utiliza lo sel ship_dict pq necesita saber el id y todo del barco del q envía la info
...
def main():
    ...
    for shipId, x, y, speed in fileShips:    
        ...
        if child == 0:  #child process. En pipes, el hijo escribe y el padre lee
            try:
                ...
                cmd = [
                    "python3",
                    os.path.join(os.path.dirname(__file__), "ship.py"),
                    "--id", str(shipId),
                    "--map", args.map,
                    "--pos", str(x), str(y),
    ...
    while ship_dict:   #while there are ships in the dictionary
        try:
            ...
            sys.stderr.flush()
            command = input("> ").strip()   #lee desde lo q se escribe en la terminal hasta el enter del usuario (up, down, lo q sea)
            ...
            for ship in ship_dict.values(): #searches for every ship in the dictionary that has a pos
                if ship.get("pos") is not None: #if that ship has a position (is alive)
                    alive += 1
            print(f"number of ships alive: {alive}", file=sys.stderr)
            sys.stderr.flush()
        except OSError as e:
            print(f"Error happened: {e}", file=sys.stderr)
            sys.exit(1)
